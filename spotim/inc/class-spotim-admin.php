...
//		if (!$options->get('spot_id') && self::check_error())
//			spotim_register();
		...
		if (self::$options->get('admin_notices') || self::$options->get('error'))
	...
	public static function create_admin_menu() {
		...
		$NotInitIcon = (isset(self::$options->finishedSetup) && self::$options->finishedSetup) ? '' : ' <span class="update-plugins count-1"><span class="plugin-count">1</span></span>';

		add_menu_page(
				__( 'Spot.IM Dashboard', 'wp-spotim' ),
				__( 'Spot.IM', 'wp-spotim' ).$NotInitIcon,
				'manage_options',
				$slug,
				array( __CLASS__, 'admin_dashboard_callback' ),
				$menu_icon
		);
		...
		add_submenu_page(
				...
				__( 'Settings', 'wp-spotim' ),
				'manage_options',
				"$slug--settings",
				array( __CLASS__, 'admin_settings_callback' ));
	}
	...
	public static function admin_settings_callback() {
		self::$options->require_template( 'admin-settings-template.php' );
	}

	public static function admin_dashboard_callback() {
		self::$options->require_template( 'admin-dashboard-template.php' );
	}
	...
	public static function admin_notices() {
		...
		{
			$msg = self::$options->get('admin_notices');
			$klass = 'updated';
		}
		$msg = __($msg, 'wp-spotim');
		echo "<div class='$klass notice'><p>$msg</p></div>";
	}

	private static function check_error()
	{
		$error = self::$options->get('error');
		if (empty($error))
			return true;

		return (substr($error, 0, 3) === 'WP:');
	}
}
