...
class SpotIM_UpdateComment {
    ...
    private function run_update_comment() {
        ...
        if (isset($feed->folders[0]->messages) && count($feed->folders[0]->messages) > 0 && isset($feed->total_items->approved)) {
            $total_items = $feed->total_items->approved;
            $messages = $feed->folders[0]->messages;
            foreach ($messages as $message) {
                $user_display_name = $this->get_user_display_name_by_id($message->user_id, $feed->folders[0]->users);
                $message->display_name = $user_display_name;
                if ($this->insertComment($message) == true) {
                    $inserted = ($inserted + 1);
                }
            }
            $offset = ($postData['offset'] + $this->offset);
            if (($total_items - 1) <= $offset) {
                echo json_encode(array('hasMore' => false, 'inserted' => $inserted));
            } else {
                echo json_encode(array('hasMore' => true, 'inserted' => $inserted, 'offset' => $offset));
            }
        }
        die;
    }
