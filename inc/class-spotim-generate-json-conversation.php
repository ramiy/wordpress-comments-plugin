...
class SpotIM_JSON_Conversation {
    public $post_id;
    private $comments;
    private $object;
    ...
    public function export() {
        ...
        // set URL of post
        $this->object->post_id = $this->post_id;

        $this->object->published_at = get_the_time( 'U', $this->post_id );

        // set URL of post
        $this->object->conversation_url = site_url( '?p=' . $this->post_id );

        // then get all comment IDs
        $this->object->comments_ids = array_values( wp_list_pluck( $this->get_top_level_comments(), 'comment_ID') );

        // first, we need the tree
        $this->object->tree = $this->get_tree();

        // aggregate all comments (messages)
        $this->object->messages = $this->aggregate_messages();

        // last but not least, users.
        $this->object->users = $this->aggregate_users();

        return apply_filters( 'spotim_conversation_ready', $this->object );
    }
    ...
    public function aggregate_users() {
        ...
        foreach ( $this->comments as $comment ) {
            ...
            if( ! empty( $user->user_url ) ) {
                $users[$user->ID]['website_url'] = $user->user_url;
            } else {
                $users[ $user->ID ]['website_url'] = '';
            }
        }
    ...
    public function aggregate_messages() {
        ...
        foreach ( $this->comments as $comment_id => $comment ) {
            ...
            if ( ! $comment_is_anonymous ) {
                $comments[ $comment->comment_ID ]['user_id'] = $comment->user_id;
                $comments[ $comment->comment_ID ]['anonymous'] = false;
            } else {
