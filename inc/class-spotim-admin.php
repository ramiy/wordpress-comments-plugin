...
class SpotIM_Admin {
    ...
    public function register_settings() {
        ...
        add_settings_field(
                'spot_token', __('Your Spot.IM Token', 'wp-spotim'), array('SpotIM_Settings_Fields', 'text_field'), $this->slug, 'general_settings_section', array(
            'id' => 'spot_token',
            'page' => $this->slug,
            'desc' => 'Add your Spot.IM Token.<br><a href="#" id="si_update_comment">Sync Comments From Spot.IM</a> <span class="progress-wrapper"><img src="' . plugins_url('/spotim-comments/ajax-loader.gif') . '" style="vertical-align: middle; margin-left: 15px; display: none;" /><span class="progress_count" style="vertical-align: middle; margin-left: 15px; display: none;"></span></span>'
                )
        );

        add_settings_field(
                'spot_cronjob_frequency', __('Frequency', 'wp-spotim'), array('SpotIM_Settings_Fields', 'select_field'), $this->slug, 'general_settings_section', array(
            'id' => 'spot_cronjob_frequency',
            'page' => $this->slug,
            'desc' => 'Select cron-job frequency',
            "type" => "select",
            "std" => "4",
            "options" => array("1800" => "30 min", "3600" => "1 hour", "43200" => "12 hours", "86400" => "24 hours")
                )
        );
        ...
        <div class="wrap">
            ...
            <script>
                jQuery(document).ready(function ($) {
                    function updateCommentfromSpotIm(offset, inserted) {
                        $.ajax({
                            url: ajaxurl,
                            type: 'post',
                            dataType: 'json',
                            data: {
                                'action': 'spotim_update_comment',
                                'offset': offset,
                                'inserted': inserted
                            },
                            success: function (data) {
                                var progress_count = '';
                                if (data.inserted === 1) {
                                    progress_count = data.inserted + ' Comment updated';
                                } else if (data.inserted > 1) {
                                    progress_count = data.inserted + ' Comments updated';
                                }
                                $('span.progress-wrapper .progress_count').text(progress_count).show();
                                if (data.hasMore === true) {
                                    setTimeout(function () {
                                        updateCommentfromSpotIm(data.offset, data.inserted);
                                    }, 1000);
                                } else if (data.hasMore === false) {
                                    $('#si_update_comment').removeClass('running');
                                    $('span.progress-wrapper img').hide();
                                    setTimeout(function () {
                                        $('span.progress-wrapper .progress_count').text('Your comments are all up to date').show();
                                    }, 1000);
                                }
                            }
                        });
